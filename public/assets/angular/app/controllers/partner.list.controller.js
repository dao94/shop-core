    ...
    .controller('PartnerController', [
        '$scope','$http', '$restful','$modal','growl', 'FileUploader', 'CSRF_TOKEN',  function ($scope, $http, $restful, $modal,growl, FileUploader, CSRF_TOKEN) {
            ...
            $scope.change_status = function(list) {
                ...
                $http({
                    method  : 'POST',
                    url     : base_url+'partner/changestatus',
                    data    : data,
                    dataType: 'json'
            ...
            $scope.loadPage = function (stt){
               var stt = stt || "";
                $http.get(base_url + 'partner?limit='+$scope.itemsPerPage+'&page='+$scope.currentPage+'&stt='+stt).success(function (resp){
            ...
            $scope.modalOpen_add = function (size) {
                ...
                modalInstance = $modal.open({
                    templateUrl: 'views/admin/partner/add.html',
                    size: size,
                    controller: function ($scope, $modalInstance, growl, $http, FileUploader, CSRF_TOKEN) {
                        ...
                        uploader.onCompleteItem = function(fileItem, response, status, headers) {
                            if(!response.error){
                                $scope.partner.images = response.data;
                            }
                        };
                        $scope.add_item = function() {
                            $scope.loading = true;
                            $scope.disable = true;
                            var data = {name:$scope.name,images:$scope.partner.images,link:$scope.link,alt:$scope.alt};
                            $http({
                                method  : 'POST',
                                url     : base_url+'partner/post',
                                data    : data,
                                dataType: 'json'
                                }).success(function (result){
                                    if(result.message == 'Done') {
            ...
            $scope.modalOpen_update = function (rs,index,size) {
                ...
                modalInstance = $modal.open({
                    ...
                    controller: function ($scope, $modalInstance, growl, $http, FileUploader, CSRF_TOKEN) {
                        ...
                        $scope.partner = {};
                        $scope.row = rs;
                        $scope.partner.images = rs.images;
                        var uploader = $scope.uploader = new FileUploader({
                            url         : base_url + 'upload',
                            alias       : 'newsFile',
                            formData    : [
                        ...
                        $scope.edit_item = function() {
                            $scope.loading = true;
                            $scope.disable = true;
                            var data = {id:rs.id,name:$scope.row.name,images:$scope.row.images,link:$scope.row.link,alt:$scope.row.alt};
                            $http({
                                method  : 'POST',
                                url     : base_url+'partner/push',
                                data    : data,
                                dataType: 'json'
                                }).success(function (result){
                                    if(result.message == 'Done') {
            ...
            $scope.delete = function(index,id) {
                if(confirm("Bạn muốn xóa đối tác này ?")){
                    $scope.loading = true;
                    $scope.disable = true;
                    $http({
                        method  : 'POST',
                        ...
                        dataType: 'json'
                            ...
                            if(result.message == 'Done') {
                                growl.success("Xóa thành công đối tác !");
                                $scope.disable = false;
                                $scope.loading = false;
                                $scope.loadPage();
                            }else {
